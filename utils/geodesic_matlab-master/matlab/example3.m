...
for i=1:length(stop_points);
    path{i} = geodesic_trace_back(algorithm, stop_points{i});     %find a shortest path from source to destination
    [x,y,z] = extract_coordinates_from_path(path{i});
    plot3(x,y,z);    %plot a sinlge path for this algorithm
    plot3(stop_points{i}.x,stop_points{i}.y,stop_points{i}.z,'bo','MarkerSize',3, 'MarkerFaceColor', 'b');
end;
