...
for level=1:main.subdivide
    main.kslice = ceil(dimen(3)*2^(level-1-main.subdivide));
    ...
    clear gradx grady gradz imsmall 
    % concatenate the image with its gradients
...
disp('MIRT: 3D non-rigid registration complete.')
disptime(toc);
